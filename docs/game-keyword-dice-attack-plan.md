...
4. 본문에서 멘션 태그를 제거한 나머지 텍스트를 키워드로 추출
5. `Game::KeywordLookupService.call(keyword)` 호출 → 구문 반환 (없으면 `nil`)
...
- **키워드 매칭 규칙**: 완전 일치 vs 부분 포함(예: "파이어볼 시전"처럼 키워드 앞뒤에 다른 텍스트가 붙는 경우) 여부 미정. 기본안: 멘션 제거 후 텍스트를 trim한 뒤 완전 일치.
