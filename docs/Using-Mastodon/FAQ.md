...
#### How i can change the default language of the Instance?

You can change the default language in file config/application.rb on line 28.
Change the "config.i18n.default_locale    = :en" value to anything you want. At the time of writing, supported languages are
English (en), German (de), Spanish (es), Portuges (pt), French (fr), Hungary (hu) English United-Kingdom (uk), Chinese (zh-cn) and Finnish (fi).
