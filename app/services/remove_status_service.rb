...
class RemoveStatusService < BaseService
  ...
  def call(status, **options)
    @payload  = JSON.generate(event: :delete, payload: status.id.to_s)
    @status   = status
  ...
  def signed_activity_json
    @signed_activity_json ||= JSON.generate(serialize_payload(@status, @status.reblog? ? ActivityPub::UndoAnnounceSerializer : ActivityPub::DeleteNoteSerializer, signer: @account, always_sign: true))
  end
