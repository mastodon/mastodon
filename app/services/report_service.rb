...
class ReportService < BaseService
  ...
  def reported_status_ids
    return AccountStatusesFilter.new(@target_account, @source_account, { include_groups: true }).results.with_discarded.find(Array(@status_ids)).pluck(:id) if @source_account.local?
    ...
    visibility << :group # We will need to change this if we add support for groups with private messages
