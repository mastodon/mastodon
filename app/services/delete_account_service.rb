...
class DeleteAccountService < BaseService
  ...
  def reject_follows!
    ...
    ActivityPub::DeliveryWorker.push_bulk(Follow.where(account: @account)) do |follow|
      [JSON.generate(serialize_payload(follow, ActivityPub::RejectFollowSerializer)), follow.target_account_id, @account.inbox_url]
    end
  end
  ...
  def undo_follows!
    ...
    ActivityPub::DeliveryWorker.push_bulk(Follow.where(target_account: @account)) do |follow|
      [JSON.generate(serialize_payload(follow, ActivityPub::UndoFollowSerializer)), follow.account_id, @account.inbox_url]
    end
  end
  ...
  def delete_actor_json
    @delete_actor_json ||= JSON.generate(serialize_payload(@account, ActivityPub::DeleteActorSerializer, signer: @account, always_sign: true))
  end
