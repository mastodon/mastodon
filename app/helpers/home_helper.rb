      ...
      content_tag(:div, class: 'account__wrapper') do
        section = if account.nil?
                    ...
                    link_to(path || TagManager.instance.url_for(account), class: 'account__display-name') do
                      content_tag(:div, class: 'account__avatar-wrapper') do
                        content_tag(:div, '', class: 'account__avatar', style: "width: #{size}px; height: #{size}px; background-size: #{size}px #{size}px; background-image: url(#{current_account&.user&.setting_auto_play_gif ? account.avatar.url : account.avatar_static_url})")
                      end +
                        content_tag(:span, class: 'display-name') do
                          content_tag(:bdi) do
