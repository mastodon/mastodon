...
module HomeHelper
  ...
  def group_link_to(group, button = '', path: nil)
    content_tag(:div, class: 'account') do
      content_tag(:div, class: 'account__wrapper') do
        section = if group.nil?
                    content_tag(:div, class: 'account__display-name') do
                      content_tag(:div, class: 'account__avatar-wrapper') do
                        image_tag(full_asset_url('avatars/original/missing.png', skip_pipeline: true), class: 'account__avatar')
                      end +
                    ...
                    link_to(path || ActivityPub::TagManager.instance.url_for(group), class: 'account__display-name') do
                      content_tag(:div, class: 'account__avatar-wrapper') do
                        image_tag(full_asset_url(current_account&.user&.setting_auto_play_gif ? group.avatar_original_url : group.avatar_static_url), class: 'account__avatar')
                      end +
                        content_tag(:span, class: 'display-name') do
                          content_tag(:bdi) do
                            content_tag(:strong, group_display_name(group, custom_emojify: true), class: 'display-name__html emojify')
                          end +
                            content_tag(:span, ActivityPub::TagManager.instance.uri_for(group), class: 'display-name__account')
                        end
                    end
                  end
