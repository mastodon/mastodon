...
class Api::V1::Accounts::FollowerAccountsController < ApiController
  ...
  def next_path
    if records_continue?
      api_v1_account_followers_url pagination_params(max_id: @accounts.last.active_relationships.first.id)
    end
  end

  def prev_path
    unless @accounts.empty?
      api_v1_account_followers_url pagination_params(since_id: @accounts.first.active_relationships.first.id)
    end
  end
