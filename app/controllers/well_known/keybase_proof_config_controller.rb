...
module WellKnown
  class KeybaseProofConfigController < ActionController::Base
    ...
    def show
      ...
      prefill_params = 'provider=Keybase&token=%{sig_hash}&provider_username=%{kb_username}&ua=%{kb_ua}'
      @prefill_url = "#{new_settings_identity_proof_url}?#{prefill_params}"
      @profile_url =  "#{root_url}@%{username}"
      @check_url = "#{api_v1_keybase_proofs_url}?username=%{username}"
    end
