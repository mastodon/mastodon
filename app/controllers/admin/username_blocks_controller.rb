...
class Admin::UsernameBlocksController < Admin::BaseController
  before_action :set_username_block, only: [:edit, :update, :destroy]
  ...
  def destroy
    authorize @username_block, :destroy?
    log_action :destroy, @username_block
    redirect_to admin_username_blocks_path, notice: I18n.t('admin.username_blocks.destroyed_msg')
  end
  ...
  def resource_params
    params
      .expect(username_block: [:username, :exact, :allow_with_approval])
  end
