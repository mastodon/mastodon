...
const messages = defineMessages({
  ...
  admin_moderation: { id: 'hashtag.admin_moderation', defaultMessage: 'Open moderation interface for #{name}' },
});
...
export const HashtagHeader = withIdentity(injectIntl(({ tag, intl, disabled, onClick, identity }) => {
  ...
  if (signedIn && (permissions & PERMISSION_MANAGE_TAXONOMIES) === PERMISSION_MANAGE_TAXONOMIES ) {
    menu.push({ text: intl.formatMessage(messages.admin_moderation, { name: tag.get("name") }), href: `/admin/tags/${tag.get('id')}` });
  }
