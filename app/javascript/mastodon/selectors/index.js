...
const makeGetFiltersRegex = () => {
  ...
  return (state, { contextType }) => {
    ...
    const filters = state
      .get('filters', ImmutableList())
      .filter(
        filter =>
          filter.get('context').includes(serverSideType) &&
          (filter.get('expires_at') === null ||
            Date.parse(filter.get('expires_at')) > new Date()),
      );

    if (!memo[serverSideType] || !is(memo[serverSideType].filters, filters)) {
      const dropRegex = regexFromFilters(
        filters.filter(filter => filter.get('irreversible')),
      );
...
export const makeGetStatus = () => {
  return createSelector(
    ...
    (statusBase, statusReblog, accountBase, accountReblog, filtersRegex) => {
      ...
      const dropRegex =
        (accountReblog || accountBase).get('id') !== me && filtersRegex[0];
      if (
        dropRegex &&
        dropRegex.test(
      ...
      const regex =
        (accountReblog || accountBase).get('id') !== me && filtersRegex[1];
      const filtered =
        regex &&
        regex.test(
          statusBase.get('reblog')
