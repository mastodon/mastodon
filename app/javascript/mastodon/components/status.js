    ...
    if (status.get('reblog', null) !== null && typeof status.get('reblog') === 'object') {
      ...
      prepend = (
        <div className='status__prepend'>
          ...
          <FormattedMessage id='status.reblogged_by' defaultMessage='{name} boosted {time}' values={{ name: <a onClick={this.handleAccountClick} data-id={status.getIn(['account', 'id'])} href={status.getIn(['account', 'url'])} className='status__display-name muted'><strong dangerouslySetInnerHTML={display_name_html} /></a>, time: <RelativeTimestamp timestamp={status.get('created_at')} /> }} />
        </div>
      );
