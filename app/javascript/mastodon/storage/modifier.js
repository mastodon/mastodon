    ...
    .then(db => {
      ...
      function evict(toEvict) {
        toEvict.forEach(record => {
          openCache()
            .then(cache =>
              accountAssetKeys.forEach(key => cache.delete(records[key])),
            )
            .catch(printErrorIfAvailable);

          accountsMovedIndex.getAll(record.id).onsuccess = ({ target }) =>
            evict(target.result);

          statusesIndex.getAll(record.id).onsuccess = ({ target }) =>
            evictStatusesByRecords(target.result);

          accountsIdIndex.getKey(record.id).onsuccess = ({ target }) =>
            target.result && accounts.delete(target.result);
        });
      }
...
export function evictStatuses(ids) {
  return openDB()
    .then(db => {
      ...
      ids.forEach(id => {
        reblogIndex.getAllKeys(id).onsuccess = ({ target }) =>
          target.result.forEach(reblogKey => store.delete(reblogKey));

        idIndex.getKey(id).onsuccess = ({ target }) =>
          target.result && store.delete(target.result);
      });
...
export function putAccounts(records, avatarStatic) {
  ...
  put(
    'accounts',
    records,
    (newRecord, oldKey, store, oncomplete) => {
          ...
          new Promise((resolve, reject) => {
            ...
            retrieval.onsuccess = () => {
              if (retrieval.result.length > 0) {
                resolve(
                  evictAccountsByRecords(retrieval.result).then(freeStorage),
                );
              } else {
