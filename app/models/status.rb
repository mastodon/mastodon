...
class Status < ApplicationRecord
  ...
  scope :not_reblogs_excluded_by_account, ->(account) { account.reblogs_excluded_from_timeline_account_ids.blank? ? all : where('statuses.reblog_of_id IS NULL OR statuses.account_id NOT IN (?)', account.reblogs_excluded_from_timeline_account_ids) }
  ...
  class << self
    ...
    def filter_timeline_for_account(query, account, local_only)
      ...
      query = query.not_reblogs_excluded_by_account(account)
  ...
  def filter_from_context?(status, account)
    ...
    should_filter ||= status.reblog? && account&.muting_reblogs?(status.account_id)
