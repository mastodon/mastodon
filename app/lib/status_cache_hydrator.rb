...
class StatusCacheHydrator
  ...
  def hydrate(account_id)
    ...
    if payload[:application].nil? && @status.account_id == account_id
      payload[:application] =
        if @status.application.present?
          ActiveModelSerializers::SerializableResource.new(@status.application,
                                                           serializer: REST::StatusSerializer::ApplicationSerializer).as_json
        end
    end
    ...
    if payload[:reblog]
      ...
      payload[:filtered]   = CustomFilter.apply_cached_filters(CustomFilter.cached_filters_for(account_id), @status.reblog).map do |filter|
        ActiveModelSerializers::SerializableResource.new(filter, serializer: REST::FilterResultSerializer).as_json
      end

      # If the reblogged status is being delivered to the author who disabled the display of the application
      # used to create the status, we need to hydrate it here too
      if payload[:reblog][:application].nil? && @status.reblog.account_id == account_id
        payload[:reblog][:application] =
          if @status.reblog.application.present?
            ActiveModelSerializers::SerializableResource.new(
              @status.reblog.application, serializer: REST::StatusSerializer::ApplicationSerializer
            ).as_json
          end
      end
      ...
      payload[:reblog][:muted]      =
        ConversationMute.where(account_id: account_id, conversation_id: @status.reblog.conversation_id).exists?
      payload[:reblog][:bookmarked] = Bookmark.where(account_id: account_id, status_id: @status.reblog_of_id).exists?
      if @status.reblog.account_id == account_id
        payload[:reblog][:pinned]     =
          StatusPin.where(account_id: account_id,
                          status_id: @status.reblog_of_id).exists?
      end
      ...
      payload[:muted]      =
        ConversationMute.where(account_id: account_id, conversation_id: @status.conversation_id).exists?
      payload[:bookmarked] = Bookmark.where(account_id: account_id, status_id: @status.id).exists?
      if @status.account_id == account_id
        payload[:pinned]     =
          StatusPin.where(account_id: account_id, status_id: @status.id).exists?
      end
      payload[:filtered] = CustomFilter.apply_cached_filters(CustomFilter.cached_filters_for(account_id), @status).map do |filter|
        ActiveModelSerializers::SerializableResource.new(filter, serializer: REST::FilterResultSerializer).as_json
      end
