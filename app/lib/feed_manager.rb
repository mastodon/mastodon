...
class FeedManager
  ...
  def unpush_from_home(account, status, update: false)
    ...
    redis.publish("timeline:#{account.id}", JSON.generate(event: :delete, payload: status.id.to_s)) unless update
    true
  end
  ...
  def unpush_from_list(list, status, update: false)
    ...
    redis.publish("timeline:list:#{list.id}", JSON.generate(event: :delete, payload: status.id.to_s)) unless update
    true
  end
