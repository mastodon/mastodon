...
class FeedManager
  ...
  def merge_into_home(from_account, into_account)
    ...
    query        = from_account.statuses.where(visibility: %i(public unlisted private)).includes(:preloadable_poll, :media_attachments, reblog: :account).limit(FeedManager::MAX_ITEMS / 4)
  ...
  def populate_home(account)
    ...
    account.following.includes(:account_stat).find_each do |target_account|
      ...
      statuses = target_account.statuses.where(visibility: %i(public unlisted private)).includes(:preloadable_poll, :media_attachments, :account, reblog: :account).limit(limit)
