...
class FeedManager
  ...
  def filter_from_home?(status, receiver)
    ...
    if status.reply? && !status.in_reply_to_account_id.nil?                   # Filter out if it's a reply
      ...
      should_filter = receiver.blocking_account_or_domain?(status.reblog.account)               # if I'm blocking the reblogged person
    end

    should_filter ||= receiver.blocking_account_or_domain?(status.mentions.map(&:account_id))   # or if it mentions someone I blocked
  ...
  def filter_from_mentions?(status, receiver)
    ...
    should_filter ||= receiver.blocking_account_or_domain?(status.account)                                    # or it's from someone I blocked
    should_filter ||= receiver.blocking_account_or_domain?(status.mentions.includes(:account).map(&:account)) # or if it mentions someone I blocked
    ...
    if status.reply? && !status.in_reply_to_account_id.nil?                                 # or it's a reply
      should_filter ||= receiver.blocking_account_or_domain?(status.in_reply_to_account)                      # to a user I blocked
    end
  ...
  def filter_from_public?(status, receiver)
    should_filter   = receiver.blocking_account_or_domain?(status.account)
    should_filter ||= receiver.blocking_account_or_domain?(status.mentions.includes(:account).map(&:account))

    if status.reply? && !status.in_reply_to_account_id.nil?
      should_filter ||= receiver.blocking_account_or_domain?(status.in_reply_to_account)
    elsif status.reblog?
      should_filter ||= receiver.blocking_account_or_domain?(status.reblog.account)
    end
