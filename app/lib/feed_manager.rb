...
class FeedManager
  ...
  def merge_into_home(from_account, into_account)
    ...
    query        = from_account.statuses.unrestricted_visibility.includes(:preloadable_poll, :media_attachments, reblog: :account).limit(FeedManager::MAX_ITEMS / 4)
  ...
  def populate_home(account)
    ...
    account.following.includes(:account_stat).reorder(nil).find_each do |target_account|
      ...
      statuses = target_account.statuses.unrestricted_visibility.includes(:preloadable_poll, :media_attachments, :account, reblog: :account).limit(limit)
