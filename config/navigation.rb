...
SimpleNavigation::Configuration.run do |navigation|
  navigation.items do |n|
    ...
    n.item :profile, safe_join([fa_icon('user-pen fw'), t('settings.profile')]), settings_profile_path, if: -> { current_user.functional? } do |s|
    ...
    n.item :moderation, safe_join([fa_icon('gavel fw'), t('moderation.title')]), nil, if: -> { current_user.can?(:manage_reports, :view_audit_log, :manage_users, :manage_invites, :manage_taxonomies, :manage_federation, :manage_blocks) } do |s|
      ...
      s.item :instances, safe_join([fa_icon('server fw'), t('admin.instances.title')]), admin_instances_path(limited: whitelist_mode? ? nil : '1'), highlights_on: %r{/admin/instances|/admin/domain_blocks|/admin/domain_allows}, if: -> { current_user.can?(:manage_federation) }
