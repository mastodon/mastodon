...
module Mastodon
  class StatusesCLI < Thor
    ...
    def remove
      ...
      Status.remote
            .where('id < ?', max_id)
            .where('reblog_of_id NOT IN (SELECT statuses1.id FROM statuses AS statuses1 WHERE statuses1.id = statuses.reblog_of_id AND (statuses1.uri IS NULL OR statuses1.local))') # Skip reblogs of local statuses
            ...
            .where('id NOT IN (SELECT statuses1.reblog_of_id FROM statuses AS statuses1 WHERE statuses.id = statuses1.reblog_of_id AND (statuses1.uri IS NULL OR statuses1.local OR statuses1.id >= ?))', max_id) # Skip statuses reblogged by local accounts or with recent boosts
