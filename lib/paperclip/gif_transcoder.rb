...
class GifReader
  ...
  class GifReaderException
  end

  class UnknownImageType < GifReaderException
  end

  class CannotParseImage < GifReaderException
  end
  ...
  def skip_extension_block!(s)
    if EXTENSION_LABELS.include?(s.read(1).unpack('C')[0])
      block_size, = s.read(1).unpack('C')
      s.seek(block_size, IO::SEEK_CUR)
    end
  ...
  def skip_sub_blocks!(s)
    while true do
      size, = s.read(1).unpack('C')
      break if size == 0
      s.seek(size, IO::SEEK_CUR)
    end
  end
end
