  ...
  context 'when importing bookmarks' do
    ...
    describe 'when no bookmarks are set' do
      ...
      it 'adds the toots the user has access to to bookmarks' do
        ...
        subject.call(import)
        expect(account.bookmarks.map { _1.status.id }).to include(local_status.id)
        expect(account.bookmarks.map { _1.status.id }).to include(remote_status.id)
        expect(account.bookmarks.map { _1.status.id }).to_not include(direct_status.id)
