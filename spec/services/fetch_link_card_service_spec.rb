...
RSpec.describe FetchLinkCardService do
  ...
  context 'in a local status' do
    ...
    context do
      let(:status) { Fabricate(:status, id: 1, text: 'Check out http://example.中国') }

      it 'deletes preview card fetch pending flag' do
        Redis.current.set 'preview_card_fetch:1:pending', 1
        subject.call(status)
        expect(Redis.current.exists('preview_card_fetch:1:pending')).to eq false
      end

      it 'publishes preview card fetch finish message' do
        expect(Redis.current).to receive(:publish).with('preview_card_fetch:1:progress', '1')
        subject.call(status)
      end
    end
  end
