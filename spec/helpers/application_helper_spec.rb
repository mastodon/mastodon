...
describe ApplicationHelper do
  ...
  describe 'the built-in rails translate helper' do
    subject { helper.t('generic.validation_errors', count: 1) }
    ...
    context 'with the `en` locale which has `one` and `other` plural values' do
      ...
      it 'translates to `en` without error' do
    ...
    context 'with the `my` locale which has only `other` plural value' do
      ...
      it 'translates to `my` without error' do
        expect { subject }.to_not raise_error
