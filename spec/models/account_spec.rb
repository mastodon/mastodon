  ...
  describe 'validations' do
    ...
    context 'when is local' do
      ...
      it 'is invalid if the username is longer then 32 characters' do
        account = Fabricate.build(:account, username: Faker::Lorem.characters(33))
        account.valid?
        expect(account).to model_have_error_on_field(:username)
      end

      it 'is invalid if the display name is longer than 32 characters' do
        account = Fabricate.build(:account, display_name: Faker::Lorem.characters(33))
        account.valid?
        expect(account).to model_have_error_on_field(:display_name)
      end

      it 'is invalid if the note is longer than 256 characters' do
        account = Fabricate.build(:account, note: Faker::Lorem.characters(257))
        account.valid?
    ...
    context 'when is remote' do
      ...
      it 'is valid even if the username is longer then 32 characters' do
        account = Fabricate.build(:account, domain: 'domain', username: Faker::Lorem.characters(33))
        account.valid?
        expect(account).not_to model_have_error_on_field(:username)
      end

      it 'is valid even if the display name is longer than 32 characters' do
        account = Fabricate.build(:account, domain: 'domain', display_name: Faker::Lorem.characters(33))
        account.valid?
        expect(account).not_to model_have_error_on_field(:display_name)
      end

      it 'is valid even if the note is longer than 256 characters' do
        account = Fabricate.build(:account, domain: 'domain', note: Faker::Lorem.characters(257))
        account.valid?
