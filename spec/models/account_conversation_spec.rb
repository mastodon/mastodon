...
RSpec.describe AccountConversation, type: :model do
  ...
  describe '.add_status' do
    ...
    it 'appends to old record when there is a match' do
      ...
      conversation = AccountConversation.create!(account: alice, conversation: last_status.conversation, participant_account_ids: [bob.id], last_status: last_status)
  ...
  describe '.remove_status' do
    it 'updates last status to a previous value' do
      last_status  = Fabricate(:status, account: alice, visibility: :direct)
      status       = Fabricate(:status, account: alice, visibility: :direct)
      conversation = AccountConversation.create!(account: alice, conversation: last_status.conversation, participant_account_ids: [bob.id], status_ids: [status.id], last_status: last_status)
