  ...
  describe '#translatable?' do
    ...
    context 'status language is nil' do
      it 'returns true' do
        ...
        expect(TranslationService.configured).to have_received(:supported?).with(nil, 'en').and_return(true)
        expect(subject.translatable?).to be true
      end
    end
    ...
    context 'status language is unsupported' do
      it 'returns false' do
        subject.language = 'af'
        expect(TranslationService.configured).to have_received(:supported?).with('af', 'en').and_return(false)
        expect(subject.translatable?).to be false
      end
    end
    ...
    context 'default locale is unsupported' do
      it 'returns false' do
        expect(TranslationService.configured).to have_received(:supported?).with('es', 'af').and_return(false)
