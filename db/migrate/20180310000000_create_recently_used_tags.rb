class CreateRecentlyUsedTags < ActiveRecord::Migration[5.1]
  def change
    create_table :recently_used_tags do |t|
      ...
      t.references :tag, foreign_key: { on_delete: :cascade, on_update: :cascade, primary_key: :name }, index: false, null: false, type: :string
      t.index [:account_id, :tag_id], unique: true
      t.index [:account_id, :index], unique: true
    end
  end
end
